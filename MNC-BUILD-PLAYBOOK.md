...
- **Border-radius (corrected July 16, 2026 — this replaces every earlier "zero border-radius" instruction in this doc, sitewide, no exceptions):** cards/containers 8–10px, buttons/inputs 6px, pills/tags/badges only 9999px.
- Soft box-shadow is standard for elevation: `0 10px 40px rgba(37,37,53,0.07)` — one soft, low-opacity shadow, no heavy or stacked shadows, no shadow on elements that don't need elevation
- Only Lora (display) + DM Sans (body) — no Montserrat, no Arial, no other fonts, in HTML tools
- Only the current locked color palette (`MNC_Color_Reference_July2026.html`) — no retired colors, no invented colors
...
1. Grep for `border-radius` and `box-shadow` values that don't match the current standard (cards/containers 8–10px, buttons/inputs 6px, pills/tags/badges only 9999px, shadow only `0 10px 40px rgba(37,37,53,0.07)`), and any hex color outside the locked palette (`MNC_Color_Reference_July2026.html`). List every violation with file + line number.
2. Grep for Montserrat, Arial, or any font outside Lora/DM Sans.
3. Grep for any banned word or phrase from the brand skill file — cross-check against the full cumulative list in `Skill_File_07-05-2026_v4.md`, not just a partial/remembered list.
...
use ONLY the current locked palette (`MNC_Color_Reference_July2026.html`), Lora
(display) + DM Sans (body) for this HTML tool, and the current border-radius/shadow
standard per our Design System: cards/containers 8–10px, buttons/inputs 6px,
pills/tags/badges only 9999px, soft shadow `0 10px 40px rgba(37,37,53,0.07)` for
elevation.
...
- Border-radius per current standard: cards/containers 8–10px, buttons/inputs 6px,
  pills/tags/badges only 9999px
- Soft box-shadow for elevation only: `0 10px 40px rgba(37,37,53,0.07)` — no heavy or
  stacked shadows
...
1. Grep for border-radius/box-shadow values that don't match the current standard
   (cards/containers 8–10px, buttons/inputs 6px, pills/tags/badges only 9999px,
   shadow only `0 10px 40px rgba(37,37,53,0.07)`), and any hex color not in the
   current locked palette (`MNC_Color_Reference_July2026.html`). List every
   violation with file + line number.
