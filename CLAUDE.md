...
- **Workbook Amazon link is a placeholder (found 2026-09-22, site audit):** `site/workbook.php`'s `$AMAZON_LINK` is still `https://www.amazon.com/YOUR_BOOK_LINK` — both "Buy on Amazon" buttons on the page link nowhere real. Needs Cece's actual KDP paperback listing URL; not findable anywhere else in the repo.
- **Blog comments cannot work for anyone (found 2026-09-22, site audit):** `blog-post.php`'s comment box is gated behind `isLoggedIn()`, which nothing sets to true anymore since accounts moved to the Hub 2026-09-20 (see the retired dashboard-era session system in `includes/auth.php`). Every visitor permanently sees "Log in to join the conversation," and that link goes to the Hub homepage with no way back to the post and no shared session even if she does sign up. Needs Cece's call: remove the comment feature, or wire it to real Hub login (bigger, cross-repo work) — see the fuller "dead login system" note this same audit still owes a full writeup on.
...
**2026-09-23 — Hub database backup (partial), and a duplicate CI workflow stopped emailing on every push.** The Hub's `data.db` (every member account, purchase, reflection — see Thread 0) had no backup anywhere. Confirmed with Cece nobody had signed up yet, so no real data was ever at risk. Built 7 daily cron jobs directly on the Hostinger account (not in either repo — `hosting_createAccountCronJobV1`, one per weekday, `0 9 * * 1`–`0 9 * * 0`) copying `data.db` to a fixed-name file in `~/backups/` outside the app's deployed directory, giving a rolling 7-day window that survives redeploys. Deliberately avoided `date +%...` filenames since this session couldn't confirm the Hostinger cron API escapes `%` correctly — used cron's own day-of-week field instead. **Not fully verified** — the cron output-reading tool was unreliable all session (returned empty even for commands that should always print something), so a future session should confirm `~/backups/` actually has 7 populated files. Still open: no off-Hostinger copy (same-account risk only), no `nodemailer`/email infra wired up to send backups elsewhere.

Separately: found `.github/workflows/mnc-qa.yml` was a near-duplicate of `deploy.yml`'s qa-check job — same scan, but never got the `continue-on-error: true` treatment Cece asked for on 2026-09-03, so it kept failing (and emailing her) on the same 13 pre-existing, already-known violations every single push. Added the same `continue-on-error: true`.

**2026-09-22 — `/start-here` is now the sitewide front door into the Hub; direct hub.mynestchapter.com links reserved for buying and Weekly Cooking for One pulled out from under its own separate promotion (same audit session, continued).** Cece's direction: `/start-here` should work the way the old `/login` page used to — every "Log In," "Create Account," and freebie-unlock CTA sitewide should land there first (a real page explaining the Hub) rather than jumping straight to the external hub.mynestchapter.com domain, the same way `/login` itself used to be a real page before the Hub existed. Scope confirmed with Cece explicitly: **join/login/freebie CTAs route through `/start-here`; buy/purchase buttons (Shop, product pages, Workbook, homepage product cards, `hubBuyUrl()`) keep going straight to the Hub** — someone who's already decided to buy shouldn't hit an extra explainer page first. Updated every "Log In"/"Create Account" link sitewide (`includes/header.php`, `index.php`, `connect.php`, `workbook.php`), every freebie CTA (`freebies.php`, `coloring.php`, `product.php`'s free-with-account branch), and the retired `login.php`/`register.php` redirects, to point to `/start-here` instead of hub.mynestchapter.com directly. The homepage hero button — literally labeled "Start Here" but previously linking straight past the actual `/start-here` page — now finally goes there, as does the "Stay Close" section's primary "JOIN — IT'S FREE" CTA. Deliberate exception: `/start-here`'s own nav and its own "JOIN — IT'S FREE" button still link directly to the Hub — no reason to loop back to the same page you're already on.

Also: Weekly Cooking for One was getting featured as its own separate thing in two places beyond the sidebar fix from earlier this session — the Hub's own dashboard home page (`home.tsx`) had a large dedicated promotional card for it, unlike any other paid product, and `/start-here`'s new "what's inside" list had it as its own bullet next to "Shop." Cece's call: it's a Shop product, not its own category — removed the dashboard card entirely and dropped it from the `/start-here` list, so it now only appears where the other paid products do.

**2026-09-22 — Booking removed sitewide; `/start-here` rebuilt around the Hub (same page-by-page audit session).** Cece's direct call: "delete anything to do with booking a time with me." Removed entirely — `site/booking.php` (the full calendar/time-slot/PHPMailer booking system), `admin/sections/bookings.php` and its admin sidebar nav entry, the `/booking` route in `.htaccess`, and the "Book thirty minutes with me / $45" section on `connect.php` (plus the now-stale mentions of "on a call" and "thirty minutes and a phone" in that page's privacy note and closing line). Swept the whole repo afterward for anything else referencing booking, $45, or the Stripe payment link — nothing left. `/start-here` (previously orphaned, still running the pre-Hub "Thrive/Support/Encourage" three-pillar layout, including a "Talk to me directly" card pointing at the now-gutted `/connect` booking flow — the homepage retrofit on 2026-09-20 had deliberately left this page's copy untouched pending a decision on its future) is no longer orphaned in the sense of being stale: rebuilt around the Hub per Cece's direct ask, reusing the homepage's own proven Hub-pitch block (`.an-primary-block`) rather than writing new copy, plus a plain "what's inside" list (Reflections, Meaningful Days, Support Circle, Freebies, Shop, Weekly Cooking for One) matching the Hub's real sidebar. Still not linked from main nav or the homepage — that placement question is separate and untouched. Also fixed while in these files: `/login`/`/register` nav links on both `connect.php` and `start-here.php` still pointed at the old redirect-only routes instead of straight to the Hub, unlike every other page already touched this session; and one remaining banned Montserrat font instance on `start-here.php`'s quiz modal.

**2026-09-22 — All three public freebies unified onto a single "join the Hub" gate.** Page-by-page site audit found every freebie had its own separate, inconsistent access rule: the 6pm Cheat Sheet had its own one-off email-capture form on `freebies.php` (a second mailing list, separate from Hub signups); the Someday List Builder and Pick Your Mood Coloring Pages had **no gate at all** — direct links straight to their widgets (the Coloring Widget gap was already flagged in this file's old retired Gating Logic table as "needs email gate added before public launch" but never actually fixed). On top of that, two more pages bypassed even the working gate: `resources.php` linked straight to the raw 6pm widget, and the orphaned `/coloring` landing page (`coloring.php`, not linked from anywhere in-site, direct-URL only) iframed the Coloring Widget directly with zero gate of any kind. Cece's direction: every freebie should require joining the Hub (free, collects an email as part of signup) rather than a separate capture form or no gate at all — one consistent path in, one list, not several. Fixed: `freebies.php` rewritten so all three cards use the same "Join the Hub — It's Free" CTA (removed the old inline capture form/JS/`reach-subscribe.php` call entirely); `resources.php` and `6pm-cheat-sheet.php` (the old "smart redirect," whose logged-in branch was already dead per the note below) now route to `/freebies` instead of a raw widget URL; `coloring.php`'s direct iframe replaced with the same Hub CTA. Also caught and fixed along the way: Montserrat font (banned) on `freebies.php`'s labels.

**2026-09-22 — Homepage/Shop/Product/About/Blog/Resources/Workbook color and CTA audit (same session as above, page-by-page pass).** Retired-palette hex and banned Montserrat found and fixed across `about.php` (was almost entirely untouched by any prior migration), `shop.php`, `product.php`, `blog.php`, `blog-post.php`, `resources.php`. Real CTA/link bugs fixed: homepage's Cooking for One product image pointed to a filename that doesn't exist on the live server (`cooking-for-one-cover.jpg`; live file is `.png`); Cooking for One's buy button (homepage, Shop, product page) pointed at the Hub's `/#/shop` route, which never listed it — added a shared `hubBuyUrl()` helper and, in `my-nest-hub`, added Cooking for One to the Shop page (`client/src/lib/shop-items.ts` extracted as a single source of truth, was previously duplicated) reusing its existing checkout routes rather than risking a rewrite of a live payment flow; the Hub sidebar's permanent "Weekly Cooking for One" link was replaced with a dynamic "Your purchases" section that shows whatever's actually unlocked, for any product, the moment its entitlement goes active (also fixed a case where buying Cooking for One wouldn't refresh that list without a page reload). `workbook.php`'s quiz-result-email and default blog-CTA links were pointing to the thinner generic `/shop/now-what-workbook` product page instead of the real, richer `workbook.php` — fixed. `workbook.php` also had no mobile navigation at all (nav links just vanished under 768px, no hamburger) — added one matching the sitewide pattern. Found, not fixed, needs Cece: `workbook.php`'s Amazon paperback link is still a placeholder (`YOUR_BOOK_LINK`); the whole session-based login system (`includes/auth.php`) is dead sitewide (nothing sets it since Hub cutover), which makes blog comments unusable by any visitor — both logged in OPEN ITEMS above.
