...
- Know Before You Sell — specced (tagline: "What it's worth. Where it sells."), NOT built to current standard. An old build exists under the retired name `WhatsThisWorth.html` — it uses retired colors (navy/gold), Montserrat, border-radius, and real emoji icons. Do not patch it. Run it through the Build Playbook from Phase 2 (Design Plan) as a fresh build under the current name and spec.
